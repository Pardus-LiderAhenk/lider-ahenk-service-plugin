...
# This script builds the project and generates Lider (lider-service.jar) & Lider Console (lider-console-service.jar) distribution files as well as Ahenk package (service.deb)
#
# Generated files can be found under /tmp/lider-ahenk-service-plugin/
set -e
...
cd "$PRJ_ROOT_PATH"/ahenk-service
dpkg-buildpackage -b
echo "Generated Ahenk package"

EXPORT_PATH=/tmp/lider-ahenk-service-plugin
...
cp -rf "$PRJ_ROOT_PATH"/lider-service/target/lider-*.jar "$EXPORT_PATH"
cp -rf "$PRJ_ROOT_PATH"/lider-console-service/target/lider-console-*.jar "$EXPORT_PATH"
